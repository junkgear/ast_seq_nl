        ...
        with gfile.GFile(directory + '/train/dev.token.nl', mode='w') as dev_token_nl:
            with gfile.GFile(directory + '/train/dev.token.ast', mode='w') as dev_token_ast:
                for i in range(train_per, test_per):
                    ...
                    dev_token_ast.write(" ".join(ast_tokens) + b'\n')
        ...
        with gfile.GFile(directory + '/test/test.token.nl', mode='w') as test_token_nl:
            with gfile.GFile(directory + '/test/test.token.ast', mode='w') as test_token_ast:
                for i in range(test_per, len(lines)):
                    ...
                    test_token_ast.write(" ".join(ast_tokens) + b'\n')
...
def ast_sentence_to_token_ids(sentence, vocabulary, lang=None):
    """Convert a string to list of integers representing token-ids.
  ...
  """
    ...
    for word in words:
        if '_' in word:
            ids = vocabulary.get(word, UNK_ID)
            if ids == UNK_ID:
                t, v = word.split('_')
        ...
        with gfile.GFile(data_path, mode="rb") as data_file:
            with gfile.GFile(target_path, mode="w") as tokens_file:
                ...
                for line in data_file:
                    ...
                    if counter % 10000 == 0:
                        print("  tokenizing line %d" % counter)
                    token_ids = ast_sentence_to_token_ids(line, vocab)
                    tokens_file.write(" ".join([str(tok) for tok in token_ids]) + "\n")


def prepare_data(data_dir, ast_vocab_size, nl_vocab_size, lang, tokenizer=None):
    ...
    create_vocabulary(ast_vocab_path, data_dir + "/train/train.token.ast", ast_vocab_size)

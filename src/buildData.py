...
def buildVocab(filename, code_unk_threshold, nl_unk_threshold, lang):
    vocab = {
        "nl_to_num": {"UNK": UNK, "AST_START": START, "AST_END": END},
        "ast_to_num": {"UNK": UNK, "AST_START": START, "AST_END": END},
        "num_to_nl": {PAD: "UNK", UNK: "UNK", START: "AST_START", END: "AST_END"},
        "num_to_ast": {UNK: "UNK", START: "AST_START", END: "AST_END"},
    }
    ...
    for tok in tokens:
        if tokens[tok] > code_unk_threshold:
            vocab["ast_to_num"][tok] = token_count
            vocab["num_to_ast"][token_count] = tok
            token_count += 1
        else:
            vocab["ast_to_num"][tok] = UNK
...
def get_data(filename, vocab, dont_skip, max_code_length, max_nl_length):
    ...
    for line in open(filename, 'r'):
        ...
        if dont_skip or (
                len(datasetEntry["code_num"]) <= max_code_length and len(datasetEntry["nl_num"]) <= max_nl_length):
...
if __name__ == '__main__':
    ...
    vocab = buildVocab('/data/' + lang + '/train.txt', code_unk_threshold, nl_unk_threshold, lang)
    get_data('/data/' + lang + '/train.txt', vocab, False, max_code_len, max_nl_len)
    get_data('/data/' + lang + '/valid.txt', vocab, False, max_code_len, max_nl_len)
    get_data(os.environ['CODENN_DIR'] + '/data/' + lang + '/dev/dev.txt', vocab, True, max_code_len, max_nl_len)
    get_data(os.environ['CODENN_DIR'] + '/data/' + lang + '/eval/eval.txt', vocab, True, max_code_len, max_nl_len)
